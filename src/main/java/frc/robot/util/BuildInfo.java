...
public class BuildInfo {
  /**
   * Exception type thrown to indicate an invalid build info file format. This exception is intended for use in flow control and should be caught (and ignored) to avoid crashing robot code.
   */
  ...
  private static boolean lineIsHeader(String line, String header) {
    return line.startsWith("### " + header)
        && line.charAt(("### " + header + " ").length()) == '(';
  }

  /**
   * Ensures the line is a header line with the given header. If it is not, reports an error to the Driver Station and throws a {@link InvalidFormatException}. That exception is intended only for flow control and should be caught outside the processing code.
   *
   * @param line The line to check.
   * @param header The header to check against (without the parentheses).
   */
  private static void expectLineIsHeader(String line, String header) {
    if (!lineIsHeader(line, header)) {
      DriverStation.reportError("Invalid build info file!", true);
      throw new InvalidFormatException();
    }
  }

  /**
   * Processes lines from the scanner up to and including the line with the given header. After this function finishes, the scanner's position will be immediately after the line with the header.
   *
  ...
  /**
   * Logs the build info from the file in the deploy directory. All build info goes to stdout, and the git information is also published to NetworkTables under the BuildInfo table.
   */
  public static void logBuildInfo() {
    ...
    if (buildInfo.isPresent()) {
      try (Scanner scanner = new Scanner(buildInfo.get())) {
        ...
        expectLineIsHeader(scanner.nextLine(), "Commit date");
        ...
        remotes = scanner.nextLine().split(", ");
        // Read the changed files
        expectLineIsHeader(scanner.nextLine(), "Changed files");
        ...
        readToHeader(scanner, "", (line) -> untrackedFilesList.add(line));
